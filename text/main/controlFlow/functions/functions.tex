...
As sketched in \cref{fig:pipInstallPytest}, we then type in \bashil{pip install pytest pytest-timeout}\pythonIdx{pip} and hit \keys{\enter}.~(Normally, we would do that in a \pgls{virtualEnvironment}, which we discuss later in \cref{sec:pipAndVenv}.)
