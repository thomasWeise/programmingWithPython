...
We develop a new \pythonil{sqrt} implementation in file \programUrl{exceptions:sqrt_raise} given as \cref{lst:exceptions:sqrt_raise}.
We first check whether either \pythonil{not isfinite(number)}\pythonIdx{isfinite} or \pythonil{number < 0.0} holds.
...
The latter can be observed in program \programUrl{exceptions:use_sqrt_raise} shown as \cref{lst:exceptions:use_sqrt_raise}.
...
%
\gitLoadPython{exceptions:test_sqrt_raise}{}{exceptions/test_sqrt_raise.py}{}%
\listingPython{exceptions:test_sqrt_raise}{%
...
%
\gitExec{exec:exceptions:test_sqrt_raise:pytest}{\programmingWithPythonCodeRepo}{.}{_scripts_/pytest.sh exceptions test_sqrt_raise.py}%
\listingToolOutput{exceptions:test_sqrt_raise:pytest}{%
...
We also write two \pglspl{unitTest} into the new file \programUrl{exceptions:test_sqrt}.
