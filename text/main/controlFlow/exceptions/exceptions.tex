...
First, the path to its module is given (ending in \textil{06_exceptions/sqrt_raise.py}) and it is pointed out that the exception was raised in line~15.
...
If our function raises\pythonIdx{raise} a certain exception in some case, then we should have a unit test that checks if this \pythonilsIdx{Exception} is actually raised as it should.
...
If we want to check whether a function indeed raises a certain exception of type \pythonil{ExpectedExceptionType} for a given input, then we can wrap the corresponding function call into a \pythonil{with raises(ExpectedExceptionType):}.
We will learn the exact structure and functioning of \pythonilIdx{with} blocks later.
They are closely related to exception handling, but require knowledge of some other topics that we did not yet cover.
...
This is basically a function that can be invoked in the \pythonilIdx{with} statement and that tells the \pytest\ system that, in the following, indented, block, we expect that a certain exception must be raised.
