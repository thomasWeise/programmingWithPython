...
The most primitive concept is the \pythonilIdx{Iterator}~\cite{PEP234}.
...
\pythonilIdx{Generator} functions and expressions are special \pythonilsIdx{Iterator} allowing us more control and a simpler syntax for defining element sequences, respectively.
...
We can invoke \pythonil{iter(coll)} on a collection \pythonil{coll} implementing this \pythonilIdx{Iterable} iterface and will get an \pythonilIdx{Iterator}.
Whenever we iterate over a list, a new \pythonilIdx{Iterator} is created this way.

We can also create collections like lists, sets, and dictionaries via so-called \emph{comprehension}, which basically means to write a \pythonil{for} loop \emph{into} the corresponding literal.
...
\listingBox{exec:iteration:range_iteration}{Manually iterating over a \pythonilIdx{range}, in exactly the same way that we used in \cref{lst:exec:iteration:list_iteration}.}{,style=python_console_style}%
%
\begin{sloppypar}%
...
of~\pythonilIdx{Iterable}\pythonIdx{typing.Iterable}.
...
The \pythonilIdx{min} function accepts a sequence of elements and returns the smallest one.
...
This means that the 999\decSep999\decSep996 elements are never created.
