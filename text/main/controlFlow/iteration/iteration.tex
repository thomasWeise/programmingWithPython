...
%
\begin{pythonSyntax}
[item for item in sequence]  # which is equivalen to `list(sequence)`
# or
...
Some simple examples for list comprehension.}%
%
...
The output of \ruff\ for the examples for list comprehension in \cref{lst:iteration:simple_list_comprehension}: The \pythonilIdx{for} loop constructing the list \textil{squares_1} can indeed by replaced by a list comprehension.}%
%
\begin{sloppypar}%
...
\end{sloppypar}%
%
In \cref{exec:iteration:simple_list_comprehension:ruff}, we present the output that our \cref{ut:ruff} produces when we apply it to \cref{lst:iteration:simple_list_comprehension}.
...
Anyway, in order to test whether list comprehension is really faster than iterative list construction via \pythonilIdx{append}\pythonilIdx{list!append}, we now write the second program \cref{lst:iteration:list_of_numbers_comprehension}.
