...
In the previous section, we implemented a generator function returning the endless sequence of prime numbers in file \programUrl{iteration:prime_generator} in \cref{lst:iteration:prime_generator}.
...
The answer can be found in our program \programUrl{iteration:filter_takewhile} given in \cref{lst:iteration:filter_takewhile}.

\pythonilIdx{takewhile} is a function with two parameters~\cite{PSF:P3D:TPSL:IFCIFEL}.
The second parameter is an \pythonilIdx{Iterable}.
Let's say that this \pythonilIdx{Iterable} provides a sequence of elements of some type~\pythonil{T}.
...
Then, \pythonilIdx{takewhile} constructs a \emph{new} \pythonilIdx{Iterator}, which returns the elements from the original \pythonilIdx{Iterable} as long as the predicate function returns~\pythonil{True} for them.
As soon as it hits an element from the original \pythonil{Iterable} for which the predicate returns \pythonil{False}, it will stop the iteration.
...
This is a practical example where \pythonilsIdx{lambda} come in especially handy.
Therefore, the answer to \inQuotes{How can I extract all the numbers less than~50 from the prime sequence?} is simply to call \pythonil{takewhile(lambda z: z < 50, primes())}.
This sequence is now no longer infinitely long and can conveniently be converted to a \pythonil{list}.

The built-in function~\pythonilIdx{filter} works quite similarly~\cite{PSF:P3D:TPSL:BIF}.
It, too, accepts a predicate and an~\pythonilIdx{Iterable} as input.
...
We again implement the predicate as \pythonilIdx{lambda}.
Since there underlying \pythonilIdx{Iterator} given by \pythonil{primes()} is infinite, we again use \pythonilIdx{takewhile} to limit the sequence to those primes that are less than~1000.
We pass the result of our construct to the function \pythonilIdx{tuple}, which creates an immutable and indexable sequence.
...
%
Another important utility function when dealing with sequences is the built-in function~\pythonilIdx{map}~\cite{PSF:P3D:TPSL:BIF}.
We explore its use in \programUrl{iteration:map} given as \cref{lst:iteration:map}.
...
The first argument to \pythonilIdx{map} is a function that should be applied all of the elements in the \pythonilIdx{Iterable} passed in as its second argument.
The result of \pythonilIdx{map} is a new \pythonilIdx{Iterator} with the return values of this function.

In \programUrl{iteration:map} given \cref{lst:iteration:map}, we first split \pythonil{csv_text} at all~\pythonil{\",\"}.
We then translate the elements of the resulting list to \pythonilsIdx{int} via \pythonilIdx{map}.
Finally, we \pythonilIdx{filter} the sequence to retain only values greater than~20.
We can conveniently iterate over the resulting filtered and mapped sequence using a~\pythonilIdx{for}~loop.

How about we now obtain all the squares of the values in the \pgls{CSV} data, but each value only once.
...
Then we map these chunks to integers and return their squares using the \pythonilIdx{map}~function, but this time we provide a~\pythonilIdx{lambda} that does the transformation.
...
\pythonil{max}~will then iterate over this sequence and return the largest value it encountered.
...
%
\begin{sloppypar}%
As last example for sequence processing we play a bit with the \pythonilIdx{zip} function~\cite{PSF:P3D:TPSL:BIF}.
This function accepts several \pythonilsIdx{Iterables} as arguments and returns a new \pythonilIdx{Iterator} which steps through all of input iterables in synch, returning tuples of with one value of each of them.
For example, \pythonil{zip([1, 2, 3], [\"a\", \"b\", \"c\"])} returns an \pythonilIdx{Iterator} that produces the sequence~\pythonil{(1, \"a\")}, \pythonil{(2, \"b\")}, and \pythonil{(3, \"c\")}.
...
\end{sloppypar}%
%
In \programUrl{iteration:zip}, provided as \cref{lst:iteration:zip}, we use \pythonilIdx{zip} to implement a function \pythonil{distance} that computes the Euclidean distance of two $n$\nobreakdashes-dimensional vectors or points~\pythonil{p1} and~\pythonil{p2}.
...
It uses tuple unpacking to extract the two elements~\pythonil{a} and \pythonil{b} from each of the tuples that \pythonilIdx{zip} creates.
...
With their \pythonilIdx{yield} statement, they allow us to write functions that perform a computation, pass the result to their output and allow the calling code outside to process them.
Different from normal functions, these generator function can then resume their execution until they return more results via \pythonilIdx{yield} or reach the end of their sequence.
...
This is possible because all of the sequence \pgls{API} boils down to two basic components: \pythonilIdx{Iterator} and \pythonilIdx{Iterable}.
An \pythonilIdx{Iterable} is an interface supported by any object whose elements can be accessed one-by-one.
An \pythonilIdx{Iterator} is a single pass, a single such access sequence.%
\endhsection%
%
\endhsection%
%
