...
\listingPythonAndOutput{dunder:str_vs_repr}{%
...
In \cref{lst:dunder:point_user_2} showing program \programUrl{dunder:point_user_2}, we create three instances of this class.
...
\listingPythonAndOutput{dunder:point_with_dunder_user}{%
...
\listingPythonAndOutput{dunder:point_with_hash_user}{%
Using the new \pythonil{Point} class from \cref{lst:dunder:point_with_hash} in sets and dictionaries.}{}%
%
Our \pythonil{Point} objects are immutable.
They just consist of two numerical coordinates.
Maybe there could be an application where someone would like to use them as keys for a dictionary.
Or maybe somebody would like to construct a set of points.
For this to be possible, three things are needed:%
%
\begin{enumerate}%
%
\item Instances of \pythonil{Point} must be immutable. %
This is true already. %
I just wanted to say it again.%
%
\item There must be a dunder method \dunder{eq} that compares an instance of \pythonils{Point} for equality with other objects. %
This, we already have done in \cref{lst:dunder:point_with_dunder}.%
%
\item The dunder method \dunder{hash} must be implemented that returns the hash value of a \pythonil{Point} in form of an~\pythonil{int}. %
This one is still missing.%
%
\end{enumerate}%
%
If we can fulfill these criteria, \pythonils{Point} can be elements of sets or keys in dictionaries.
So we only have to implement one more method, namely \dunder{hash}.
For the two dunder methods \dunder{eq} and \dunder{hash]}, it must hold that~\cite{PSF:P3D:TPLR:DM:H}%
%
\begin{equation}%
...
\end{equation}%
%
This is equivalent~\cite{PSF:P3D:TPSL:BIE,PSF:P3D:TPLR:DM:H} to:%
%
\begin{equation}%
...
The internal tables could be represented as linear lists.
...
They have an element-wise read, search, and update time complexity of~\bigOb{1}~\cite{H2025PM:PBOTTCODDSIPP33,PSF:TPW:TC,N2022CCSFPO}.
An element in a list~\pythonil{l} can be searched in \bigOb{\pythonil{len(l)}} instead, i.e., finding one paricular element is much slower.

As said, you can imagine that a hash table uses something like a list as backing store.
How can we turn a list~\pythonil{l} with search complexity \bigOb{\pythonil{len(l)}} into a hash table with search complexity~\bigOb{1}?
For now, let's imagine that we want to store only integers and that our list is much larger than the number of elements that we want to store.
We use a list of \pythonil{int | None} values and it is filled initially with only \pythonil{None}.
If we want to store an integer~\pythonil{i}, we would compute the \pgls{modulodiv} \pythonil{i \% len(l)} an just place it at that index.
If we want to check whether an integer~\pythonil{j} is in the list, we again compute \pythonil{j \% len(l)} and check whether it is equal to the element at that index~(using \dunder{eq}).
Both operations, insertion and search, now work in~\bigOb{1}.
Of course, this is a very coarse simplification~\cite{G2020PHTUTH,L2011PDI}.
There could be two different integers with the same result for \pythonil{i \% len(l)}, for example.
Dictionaries and sets need to handle such collisions.
They also need to grow if they begin to fill up, and so on.
...
It does not have to be a bijective mapping, i.e., it only needs to go in the direction of \pythonil{int} but not back.
It does not have to be injective, i.e., some different objects can have the same hash code.
However, it should ideally be as injective as possible, i.e., the fewer objects have the same hash code, the better.
The dictionary/set implementation is then responsible for translating these integer hash codes into valid indices into its internal table.

So the hash values are needed to find the objects in the dictionaries and sets.
...
Then \pythonil{s} translates the hash value to an index.
...
Dictionaries work the same, but instead store key-value pairs, where the hash values of the keys are computed to find the right places in the internal tables.
You can read about the details in these very interesting sources here~\cite{G2020PHTUTH,L2011PDI}.
...
Otherwise, it could be that \pythonil{\"123\" in s} is \pythonil{True} for the string \pgls{literal}~\pythonil{\"123\"}, but \pythonil{False} for \pythonil{str(123) in s}.
That would make no sense at all.

Overall, the following issues need to be considered when making objects \inQuotes{hashable.}
...
%
\bestPractice{hash}{%
For implementing \dunder{eq} and \dunder{hash}, the following rules hold~\cite{PSF:P3D:TPLR:DM:H}:%
\sloppy%
...
%
\item The results of \dunder{eq} and \dunder{hash} must be computed using the exactly same attributes. %
...
In \cref{lst:dunder:point_with_hash} outlining file \programUrl{dunder:point_with_hash}, we modify the \pythonil{Point} class from \cref{lst:dunder:point_with_dunder}.
We retain the implementation of~\dunder{eq}.
All we have to do is to add the method~\dunder{hash}.
...
%
\begin{sloppypar}%
When we were writing this, we suddenly get startled.
We permitted the coordinates of our points to be either \pythonils{int} or \pythonils{float}.
As we know, \pythonil{5.0 == 5} is \pythonil{True}.
Therefore, \pythonil{Point(5.0, 3).\_\_eq\_\_(Point(5, 3))} is also~\pythonil{True}.
But is \pythonil{hash((5.0, 3))} really the same as \pythonil{hash((5, 3))}?
Or, in simpler terms, does \pythonil{hash(5.0) == hash(5)} really hold?%
\end{sloppypar}%
%
If not, then we could violate the contract of \dunder{hash} and \dunder{eq} from~\cref{bp:hash} in a very subtle and unexpected way.
\pythonil{Point(5.0, 3)} would be equal to \pythonil{Point(5, 3)}, but their hash codes would differ.
If we have a set~\pythonil{s} of points and store \pythonil{Point(5.0, 3)} in this set, then the outcome of the query \pythonil{Point(5, 3) in s} could either be \pythonil{True} or \pythonil{False}.
If the layout of the internal table -- whose size depends on all previous insertions and deletions -- is such that the different hash codes of both objects are transformed to the same index, then the outcome would be \pythonil{True}.
In the much more likely case that they don't, the outcome is \pythonil{False}.

Thus, if \pythonil{hash(5.0) != hash(5)} was true, then our implementation could yield programs that behave unexpectedly differently, in rare situations, which we might not be able to deterministically reproduce.
That's the one of the worst kinds of bugs we can imagine.
At the same time, we could create sets that contain equal elements multiple times.
Which, in turn, would violate the definition of sets.
No wonder that \python\ is rather strict with the rules imposed on \dunder{hash} and \dunder{eq}{\dots}
...
We use our new variant of the \pythonil{Point} class in~\cref{lst:dunder:point_with_hash_user} with program \programUrl{dunder:point_with_hash_user}.

We again first create three points \pythonil{p1 = Point(3, 5)}, \pythonil{p2 = Point(7, 8)}, and \pythonil{p3 = Point(3, 5.0)}.
\pythonil{p1 == p2} is \pythonil{False}, while \pythonil{p1 == p3} is \pythonil{True}, despite the fact that the \pythonil{y}\nobreakdashes-coordinate of \pythonil{p1} is an~\pythonil{int} and the one of~\pythonil{p2} is a~\pythonil{float}~(but with the same value).

Then we create the set~\pythonil{points} as~\pythonil{\{p1, p2, p3\}}.
It has the size~2.
...
This is because \pythonil{p1} and \pythonil{p3} are equal and also have the same hash value.
...
We associate value \pythonil{\"A\"} with key~\pythonil{p1} and \pythonil{\"B\"} with key~\pythonil{p2}.
We then insert another key-value mapping by storing \pythonil{\"C\"} under key \pythonil{Point(7, 9)}.
This new key-value pair also appears in the dictionary as expected.
When we store the value \pythonil{\"D\"} under key \pythonil{Point(3.0, 5.0)}, then this overwrites the value \pythonil{\"A\"} stored under \pythonil{p1}, because \pythonil{p1} is \pythonil{Point(3, 5)}.
Querying \pythonil{point_vals[p1]} now yields \pythonil{\"D\"}.

The subject of making objects hashable is not something that we encounter very often.
But it is a topic that shows that working with dunder methods requires special care.
We should really understand what we are doing.
Otherwise, we could create very hard to understand errors.
We could implement \dunder{eq} and  \dunder{ne} by accident such that  \pythonil{x == y} is no longer the opposite of \pythonil{x != y}.
Or we could implement \dunder{eq} and \dunder{hash} such that sets are no longer set{\dots}
Such bugs would be very hard to detect.
So we always have to read the documentation thoroughly.%
%
\FloatBarrier%
\endhsection%
%
...
Well, we will need to introduce some sort of stopping criterion, but given that{\dots}
{\dots}We could compute $\sqrt{2}$~accurate to 700~digits!

Except, so far, we do not have anything that would print these digits.
Our \dunder{str} method returns fractions as text of the form~\pythonil{\"a/b\"}.
...
In file \programUrl{dunder:fraction_decimal_str_err:part_6}, we implement a new variant of our class \pythonil{Fraction}.
In \cref{lst:dunder:fraction_decimal_str_err:part_6}, we show an excerpt of this file, presenting the part of our class \pythonil{Fraction} that contains the code converting the fraction to a decimal string.
...
We begin our method \pythonil{decimal_str} by first copying the \pgls{numerator} into a variable~\pythonil{a}.
...
The Boolean variable \pythonil{negative} is set to \pythonil{True} if \pythonil{a < 0} and to \pythonil{False} otherwise.
We then make sure that \pythonil{a} is positive by setting it to~\pythonil{abs(a)}.
Then we also copy the \pgls{denominator} into a variable~\pythonil{b}.

We will not change the local variables \pythonil{negative} and~\pythonil{b} in our method, so we mark them both with the \pgls{typeHint}~\pythonilIdx{Final}.%
%
...
%
In a \pythonil{while}~loop, we now fill a list~\pythonil{digits} with the digits representing the fraction.
The loop will continue until either \pythonil{a == 0} or our list \pythonil{digits} contains \pythonil{max_frac} decimals.
...
So the first \inQuotes{digit} we append to \pythonil{digits} is~\pythonil{11}.
...
\pythonilIdx{\%} is the \pgls{modulodiv}.
Thus, \pythonil{a \% b} gives us the remainder of the division of~179 by~16, namely~3.
Thus, we get~\pythonil{a = 30}.

In the second iteration, \pythonil{a // b}, i.e., \pythonil{30 // 16}, gives us the next digit~\pythonil{1}.
Now, \pythonil{10 * (a \% b)} becomes~140 as the new value of~\pythonil{a}.
...
It stops if we can represent the fraction completely and exhaustively as decimal string with no more than the specified number \pythonil{max_frac} of digits.
This was the case in our example.
The loop also stop if we reach the maximum number of fractional digits, i.e., if \pythonil{len(digits)} exceeds \pythonil{max_frac}.

But this can also cause problems.
For example, what happens if I want to represent~$\frac{10006}{10000}=1.0006$ with only \emph{3} fractional digits.
...
Well, all we need to do to fix this is to check whether the next digit that we would not append to \pythonil{digits} would be greater or equal to~5.
If so, we increment the last digit in \pythonil{digits} by~1.
(This idea will later turn out to be an error\dots)
Anyway, to introduce this rounding behavior, we insert an \pythonil{if (a // b) >= 5} which does \pythonil{digits[-1] += 1} if its condition is met.

We now have a representation of the fraction as a list of digits.
...
Otherwise, we need to have a decimal dot~\inQuotes{.} after the first number in \pythonil{digits}.
...
If the fraction was \pythonil{negative}, we place a minus sign before the list contents, via \pythonil{digits.insert(0, "-")}\pythonIdx{list!insert}.
...
\pythonilIdx{map} returns an \pythonilIdx{Iterator} that returns the results of the \pythonilIdx{str} function applied to all the elements in \pythonil{digits} one by one.
\pythonilIdx{str} applied to a string just returns the string itself.
Applied to an integer, it converts it to a string.

The method \pythonilIdx{join}\pythonIdx{str!join} of a string concatenates all elements of the \pythonilIdx{Iterable} it receives as parameter and places the string itself as separator.
...
This can be done by placing some \pglspl{doctest} into the \pgls{docstring} of the method.
As test cases, we choose several normal situations as well as several corner cases.
...
$\frac{1}{3}$ to ten digits after the dot should yield~\pythonil{\"0.3333333333\"}.
...
This means that we need to round-up, which would make the second-to-last digit~(9) to also be rounded up, causing the next 9 to toggle as well.
...
Rounding 0.9995 to four digits will round-up the 5, which will cause all the 9s to be rounded up, too.
We finally should get result~\pythonil{\"1\"}.
...
We open our source file \programUrl{dunder:fraction_decimal_str_err:part_6} and scroll to our method \pythonil{decimal_str}.
...
When I explained how \pythonil{decimal_str} works, I used~$\frac{-179}{16}$ as an example and explained what the method would do.
Would it not be nice if we could actually step-by-step execute the program for the test?
Then we could see what it \emph{actually} does?
...
First, we again open our file \programUrl{dunder:fraction_decimal_str_err:part_6} in \pycharm\ and locate our function \pythonil{decimal_str}~(\cref{fig:dunder:debug01}).

On the left side of our code window, there is a column with the line numbers.
...
We want our program to pause right at the beginning of \pythonil{decimal_str}.
Therefore, we place the \pgls{breakpoint} there~(\cref{fig:dunder:debug02}).
...
The \pglspl{doctest} will now be executed.
Instead of running them completely, the \pgls{debugger} kicks in:
...
We can activate it with the right mouse button on the top of this row and then drag it upwards.
...
We see that \pythonil{max_frac} has the (default) value \pythonil{100}.
...
Our first test case was \pythonil{Fraction(124, 2).decimal_str()}, so the normalized fraction is indeed~$\frac{62}{1}$~(\cref{fig:dunder:debug06}).%
%
\clearpage%
\begin{figure}%
\ContinuedFloat%
...
We know that this test case will be completed successfully.
We thus are not interested in it.
We click on the \pycharmDebuggerResume\ symbol in \menu{Debug} register, which will let the program continue its execution~(\cref{fig:dunder:debug07}).
Alternatively, we can hit \keys{F9}, which has the same effect.

The execution of the \pglspl{doctest} is resumed.
It again pauses at our \pgls{breakpoint}~(\cref{fig:dunder:debug08}).
This time, we can see that we have arrived at the beginning of the second \pgls{doctest} case with \pythonil{Fraction(1, 2)}.
This test case, too, is not interesting.
So we again either click \pycharmDebuggerResume\ or press~\keys{F9} to resume execution.
...
As we already know, this test case is again one that will succeed.
It will not give us any useful information.
We can skip it as well by pressing~\keys{F9}.

The next time we reach the \pgls{breakpoint} is for the fourth \pgls{doctest} case, $\frac{-101001}{100000000}$~(\cref{fig:dunder:debug10}).
...
The fraction~$\frac{91995}{100000}$ got normalized to~$\frac{18399}{20000}$ in the initializer~\dunder{init}.
...
Right now, the \pgls{debugger} has paused the execution right at the very first line of this method.
...
The next line of code is marked and we press~\pycharmDebuggerStepOver\ to execute it with~\keys{F8}~(\cref{fig:dunder:debug17}).
...
And they should be, since \pythonil{a} is \pythonil{18399}, \pythonil{len(digits)} is~0, and \pythonil{max_frac} is~3.
...
In \cref{fig:dunder:debug24}, the loop condition is still met, so the first line in the loop body is marked again.
...
In the loop iteration after that, \pythonil{9} gets appended to \pythonil{digits} and \pythonil{a} gets updated to \pythonil{190000}.
...
When we hit \keys{F8}, the loop condition is evaluated again.
This time it evaluates to~\pythonil{False}.
The loop terminates and the mark is placed on the next line of code after the loop in~\cref{fig:dunder:debug33}.
...
So far, we got the digits~0, 9, 1, and~9 in~\cref{fig:dunder:debug34}.
...
We press the \pycharmDebuggerStepOver~button to find it out.

A look at \cref{fig:dunder:debug35} reveals the bug, i.e., the error in our code:
...
In \cref{lst:dunder:fraction_sqrt} with file \programUrl{dunder:fraction_sqrt}, we just copy the code of the \pythonil{sqrt} function from this file.
Then we apply some necessary changes.
We will basically replace all \pythonils{float} in this code with \pythonils{Fraction}.%
%
\begin{sloppypar}%
The function's \pgls{signature} changes from~\pythonil{def sqrt(number: float) -> float} to \pythonil{def sqrt(number: Fraction) -> Fraction}.
Well, we know that the square roots of some numbers like~$\sqrt{2}$ are irrational.
This means that we cannot represent the exactly with an instance of~\pythonil{Fraction}.%
\end{sloppypar}%
%
In the original code, this is not a problem.
Due to the limited range of~\pythonils{float}, the function will eventually stop.
This happens when all of the 15 to 16 digits of precision are exhausted.

However, using \pythonil{Fraction}, the same code could loop forever.
We have a precision limited only by the memory of the computer.
So the function could forever try to find better and better approximations.

Thus we need to limit the number iterations with an additional parameter.
Thus, we add the parameter \pythonil{max_steps: int = 10}.
...
In \cref{lst:dunder:fraction_sqrt}, we replace the numbers~\pythonil{0.0}, \pythonil{0.5}, and~\pythonil{1.0} with our constants~\pythonil{ZERO}, \pythonil{ONE_HALF}, and~\pythonil{ONE}.
We need to do this because our mathematical dunder methods only work with other instances of~\pythonil{Fraction}.
We could have easily implemented them to also support working with instances of~\pythonil{int} or~\pythonil{float}.
We did not do that because otherwise, our example code would have been much longer.
Either way, all the numerical constants in \cref{lst:dunder:fraction_sqrt} are now instances of~\pythonil{Fraction}.

Our new \pythonil{sqrt} function begins with an initial check whether the input number is negative.
If so, raises an~\pythonilIdx{ArithmeticError}.
After that, it has almost the same loop and body as our original variant in~\cref{lst:functions:my_math}.
The only difference is that we decrement \pythonil{max_steps} by~1 in each iteration.
We \pythonil{break} out the loop once it reaches~\pythonil{0}.%
%
\begin{sloppypar}%
...
Then it translates the result to a decimal string with \emph{750 fractional digits}.
We take the correct value from~\cite{S2024DEOSRO2,NB1997TSROTT1MD}.
Unless our function provides this number of digits correctly \emph{after only ten steps}, the \pgls{doctest} will fail!%
\end{sloppypar}%
%
The second \pgls{doctest} is simply to check whether~$\sqrt{4}$ renders as~2 when converted to a decimal string.
Of course, we just approximate the square root through several steps.
Thus, the value of the fraction might not actually be~2.
However, when rendered to 100~decimals of precision, it should return~\pythonil{"2"}.
...
In this section, we have learned how to use the \debugger.
The \debugger\ is one of the most important tools in the tool belt of a programmer.
The inner workings of code always become clear when going through it step-by-step.
We can set \pglspl{breakpoint} and let a process run until it hits them.
Then we can observe the values of variables.
We can execute every line of code separately.
We can see which variables change.
We can follow the control flow as it moves through the branches of alternatives and as it iterates through loops.
If our code has an error, i.e., a bug, then using the \debugger\ may be the first step to finding it.
I guess that's why it's called \debugger.
%
\FloatBarrier%
\endhsection%
%
...
The proper \pgls{typeHint} for this is \pgls{literal} is \pythonil{Literal[False]}\pythonIdx{Literal}\pythonIdx{typing!Literal}.%
%
\bestPractice{returnLiteralTypeHint}{%
If a parameter of a function can take on only some special constant values~\pythonil{X}, say certain integer or string \pglspl{literal}, the proper \pgls{typeHint} is \pythonil{Literal[X]}\pythonIdx{Literal}\pythonIdx{typing!Literal}~\cite{PSF:STWP:L,PEP586}. %
...
\listingPythonAndOutput{dunder:xml_user_print}{%
...
\listingPythonAndOutput{dunder:xml_user_file}{%
...
.3 \dunder{int}\DTcomment{convert to \pythonilIdx{int}: \pythonil{int(a)}\pythonIdx{int}~$\cong$~\pythonil{a.\_\_int\_\_()}~\cite{PEP3141}}.
.3 \dunder{float}\DTcomment{convert to \pythonilIdx{float}: \pythonil{float(a)}\pythonIdx{float}~$\cong$~\pythonil{a.\_\_float\_\_()}~\cite{PEP3141}}.
...
\dirtree{%
.1 \pythonilIdx{\_\_}.
.2 Arithmetics~\cite{PEP3141}, see \cref{sec:arithmeticsAndOrder} for selected examples.
...
.3 \dunder{trunc}\DTcomment{truncated to integer: \pythonil{math.trunc(a)}\pythonIdx{trunc}\pythonIdx{math!trunc}~$\cong$~\pythonil{a.\_\_trunc\_\_()}~\cite{PEP3141}}.
.3 \dunder{floor}\DTcomment{round towards~$-\infty$: \pythonil{math.floor(a)}\pythonIdx{floor}\pythonIdx{math!floor}~$\cong$~\pythonil{a.\_\_floor\_\_()}~\cite{PEP3141}}.
.3 \dunder{ceil}\DTcomment{round towards~$+\infty$: \pythonil{math.ceil(a)}\pythonIdx{ceil}\pythonIdx{math!ceil}~$\cong$~\pythonil{a.\_\_ceil\_\_()}~\cite{PEP3141}}.
