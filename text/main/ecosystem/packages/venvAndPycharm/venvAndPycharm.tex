...
%
\subfloat[][%
Since I selected the folder where the other files (\textil{requirements.txt}, \programUrl{packages:numpy_user}, \dots) are already located, \pycharm\ asks whether this is OK. %
...
%
\subfloat[][%
It contains the one line \textil{numpy==2.5.2}, which means that our project requires \numpy\ in version~2.5.2.%
...
%
\subfloat[][%
We now click on \programUrl{packages:numpy_user} and get informed that required \pglspl{package} are missing. %
...
%
\subfloat[][%
We right-click on \programUrl{packages:numpy_user} and click on~\menu{Run \inSQuotes{numpy\_user.py}}.%
...
When we click on the file \programUrl{packages:numpy_user} to open it in~\cref{fig:venvPycharm5}, we notice a yellow bar at the top of the file's contents.
...
The small overlay also tells us that \numpy\ was installed in version~2.5.2, as prescribed by the \textil{requirements.txt} file.
...
As final check that everything went well we run our program \programUrl{packages:numpy_user} in \cref{fig:venvPycharm7}.
