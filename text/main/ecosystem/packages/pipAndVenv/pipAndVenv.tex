...
%
\subfloat[][%
Now we can execute the \python\ program \programUrl{packages:numpy_user} from \cref{lst:packages:numpy_user} via \bashil{python3 numpy_user.py} + \keys{\return}.%
...
%
\subfloat[][%
Now we can execute the \python\ program \programUrl{packages:numpy_user} from \cref{lst:packages:numpy_user} via \batil{python numpy_user.py} + \keys{\return}.%
...
%
\begin{sloppypar}%
We can now run our program \programUrl{packages:numpy_user} by typing \batil{python numpy_user.py} and hitting~\keys{\return}.
