...
\pythonil{list(num_str.keys())} gives us all the keys in the dictionary \pythonil{num_str} as a \pythonilIdx{list}, which thus equals \pythonil{[2, 1, 3, 4]}.
Similarly, \pythonil{list(num_str.values())} gives us all the values in the dictionary \pythonil{num_str} as a \pythonilIdx{list}, which thus equals \pythonil{["two", "one", "three", "four"]}.
Finally, \pythonil{num_str.items()} returns the key-value pairs a sequence of tuples and wrapping this into a list yields \pythonil{[(2, "two"), (1, "one"), (3, "three"), (4, "four")]}.
...
This is because dictionaries, different from sets, are ordered data structures.
