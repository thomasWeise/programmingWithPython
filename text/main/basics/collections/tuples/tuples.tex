...
When you need to use an indexable sequence of objects, use a list only if you intent to modify this sequence. %
...
\end{sloppypar}%
%
There is an order defined on tuples~(and lists, too), meaning that they support the all six comparison operators \pythonilIdx{<}, \pythonilIdx{<=}, \pythonilIdx{==}, \pythonilIdx{>=}, \pythonilIdx{>}, and~\pythonil{!=}\pythonIdx{"!=}.
...
Only put immutable objects into tuples if you want to use the tuples as immutable objects. %
...
Violating this assumption can lead to strange errors down the line.%
}%
%
While we should usually put only immutable objects into tuples, there is one notable exception:
Later we will learn how to program functions~(in \cref{sec:functions}).
Every function can return a single value as result, like, e.g., \pythonil{sin} or \pythonil{sqrt} do.
Sometimes, if we want to return multiple values, we can return them in a tuple.
This has the advantage that the value types are clear.
Of course, in that case, these values do not need to be immutable.
(Because functions may return lists and other mutable objects\dots)

However, if we use tuples as collections of data, we often want to make sure that they can semantically be considered as constants.
To achieve this, their values should ideally also be immutable.
