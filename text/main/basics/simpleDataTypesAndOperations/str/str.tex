...
%
\gitEvalPython{str_indexing}{}{simple_datatypes/str_indexing.py}%
\listingBox{exec:str_indexing}{Specifying string \pglspl{literal} and indexing its characters.}{,style=python_console_style}%
%
As \cref{exec:str_indexing} shows, there are two basic ways to specify a text string \pgls{literal}\pythonIdx{str!literal}:
...
%
\bestPractice{strDoubleQuote}{When defining a string \pgls{literal}, the double-quotation mark variant~(\pythonil{"..."}\pythonIdx{\textquotedbl\idxdots\textquotedbl}) may be preferred over the single-quotation mark variant~(\pythonil{'...'}\pythonIdx{\textquotesingle}).~(The \citetitle{PEP8}~\cite{PEP8} does not give a recommendation, but maybe for consistency with the~\citetitle{PEP257}~\cite{PEP257}, see also~\cref{bp:longstrDoubleQuote}.)}%
%
...
\listingBox{exec:str_basic_ops}{Some more basic string operations.}{,style=python_console_style}%
...
As \cref{exec:str_basic_ops} shows, \pythonil{"World" in "Hello World!"} yields \pythonilIdx{True}, as it checks whether \pythonil{"World"} is contained in \pythonil{"Hello World!"}, which is indeed the case.
...
\label{sec:fstrings}%
%
\gitEvalPython{str_str}{}{simple_datatypes/str_str.py}%
\listingBox{exec:str_str}{The \pythonilIdx{str} function converts objects to strings.}{,style=python_console_style}%
%
...
As \cref{exec:str_str} shows, passing the integer number~\pythonil{23} to \pythonilIdx{str}, i.e., invoking \pythonil{str(23)}, yields the string~\pythonil{"23"}.
...
%
\begin{sloppypar}%
In \cref{exec:str_fstrings}, we first consider the \pgls{fstring} \pythonil{f"\{12345678901234\}\ is a really big integer."}\footnote{
...
In \cref{exec:str_fstrings}, they are printed correctly, though.}.
...
%
\gitEvalPython{str_from}{}{simple_datatypes/str_from.py}%
\listingBox{exec:str_from}{Converting strings to other datatypes.}{,style=python_console_style}%
%
...
\cref{exec:str_from} shows that we can convert the string \pythonil{"1111"} to an integer \pythonil{1111} simply by passing it to the function \pythonilIdx{int}\pythonIdx{int!function}.
...
%
\gitEvalPython{str_escape}{}{simple_datatypes/str_escape.py}%
\listingBox{exec:str_escape}{Escaping special characters in \python\ strings.\pythonIdx{str!escaping}\pythonIdx{escaping}}{,style=python_console_style}%
%
...
The answer to this problem is \emph{\glslink{escapeSequence}{escaping}}\pythonIdx{str!escaping}\pythonIdx{escaping}~\cite{PSF:P3D:TPLR:ES}.
...
In \cref{exec:str_escape}, we present several strings with \pglspl{escapeSequence}\pythonIdx{str!escaping}\pythonIdx{escaping}.
...
A double quotation mark can be printed as \pythonil{print("\\"")}, i.e., as a string which is delimited and that then contains the \pgls{escapeSequence}~{\textbackslash\textquotedbl}\pythonIdx{\textbackslash\textquotedbl}.
It then shows up as~\textil{"} in the output.
A single quotation mark can be printed as \pythonil{print("\\'")}, i.e., via the \pgls{escapeSequence}~{\textbackslash\textquotesingle}\pythonIdx{\textbackslash\textquotesingle}.
...
The \pgls{escapeSequence}~\inQuotes{\textbackslash\textbackslash}\pythonIdx{\textbackslash\textbackslash} is converted to a single backslash and \expandafter\pythonil{print("\\\\")} writes \textil{\\} to the output.
Knowing these sequences, we can now try to \pythonil{print("\\"\\'\\\\")}.
The result printed to the output then is~\textil{"'\\}.

Another situation where \pglspl{escapeSequence} are nice is when we want to have strings that span over multiple lines\pythonIdx{str!escaping}\pythonIdx{escaping}.
...
If you want to include a horizontal tabulator in a string, the \pgls{escapeSequence}~\inQuotes{\textbackslash{t}}\pythonIdx{\textbackslash{t}} is your friend:
...
If you have a string that is too long to write on a single line but you do not want to have a linebreak inside the actual string, you can simply put a backslash, hit \keys{\return}, and continue the typing the string.
The linebreak will then be ignored entirely.
Therefore, if you print~\pythonil{print("Hello\\}, hit \keys{\return}, and then continue to write \pythonil{World!")}, this produces the output~\textil{HelloWorld!}.

\Pglspl{escapeSequence} allow us to write arbitrary text in strings.
...
The backslash-based \pgls{escapeSequence} we discussed in this section work for both \pglspl{fstring} and normal strings.%
\pythonIdx{str!escaping}\pythonIdx{escaping}%
\FloatBarrier%
\endhsection%
%
...
%
\gitEvalPython{str_multiline}{}{simple_datatypes/str_multiline.py}%
\listingBox{exec:str_multiline}{Examples of multi-line strings.}{,style=python_console_style}%
%
...
In such strings, you can insert linebreaks by hitting \keys{\return} completely normally.
You can use the \pglspl{escapeSequence} from the previous section as well.
The main use case are \pglspl{docstring}, which we will discuss later, see, e.g., \cref{bp:module:docstrings}.%
%
\bestPractice{longstrDoubleQuote}{When defining a multi-line string \pgls{literal}, the double-quotation mark variant~(\pythonil{"""..."""})\pythonIdx{\textquotedbl\textquotedbl\textquotedbl\idxdots\textquotedbl\textquotedbl\textquotedbl} is preferred over the single-quotation mark variant~(\pythonil{'''...'''}\pythonIdx{\textquotesingle\textquotesingle\textquotesingle})~\cite{PEP257,PEP8}.}%
%
...
\begin{figure}%
\centering%
\includegraphics[width=0.75\linewidth]{\currentDir/strUnicodeEscape}%
...
\label{exec:str_unicode}%
\end{figure}%
%
The memory of our computers basically stores chunks of bits of certain fixed sizes, say, bytes that are composed of 8~bit each.
...
Well, by representing each character of a text as a number.
...
Maybe the most well-known historical mapping is ASCII~\cite{ASA1963ASCII,USAS1967USCFII}, which used seven bits per character.
It therefore contained only latin characters, punctuation marks, numbers, and some control characters~(like the newline and tab characters we learned when discussing string escaping).
Since different languages use different characters -- and more than $2^7=128$ in total -- many different mappings have historically evolved and still exist today.
