...
\end{figure}%
%
Floating point numbers in \python\ can be distinguished from \pythonilIdx{int}s by having a decimal dot in their text representation, i.e., \pythonil{5.0} is a \pythonil{float} and \pythonil{5} is an \pythonil{int}.
...
Let us import a few of them, again from the \pythonil{math} module, via \pythonil{from math import sin, cos, tan, log}\pythonIdx{sin}\pythonIdx{cos}\pythonIdx{tan}\pythonIdx{log}.
...
\caption{Rounding \pythonil{float} values to \pythonil{int} values.}%
\label{fig:floatMathInConsoleRound}%
\end{figure}%
%
We already learned that a single \pythonil{float} value inside a computation that otherwise uses \pythonil{int}s basically forces the whole result to become a \pythonil{float} as well.
But maybe sometimes we want to have an \pythonil{int} as result.
Therefore, we need a \inQuotes{way back} from \pythonil{float} to \pythonil{int}.
\python\ offers several functions for this.

The first and maybe most common one is \pythonilIdx{round}.
This function accepts a \pythonil{float} and rounds it to the nearest integer.
...
\pythonilIdx{floor} rounds the \pythonil{float} it receives to the nearest integer which is less than or equal to.
...
With this, we have several ways to turn \pythonil{float}s to \pythonil{int}s.
...
Intuitively, based on its name, one would assume that \pythonil{inf} stands for \inQuotes{infinity} or~$\infty$.
However, it \emph{actually} means \emph{too big to represent as \pythonil{float} or~$\infty$}.
...
As another example, let us again import the natural logarithm function \pythonilIdx{log} and the Euler's constant~\pythonilIdx{e} from the \pythonil{math} module by doing \pythonil{from math import e, log}\pythonIdx{from}\pythonIdx{math}\pythonIdx{import}.
...
Finally, \pythonilIdx{inf} also exists as constant in the \pythonil{math} module.
...
\caption{Not-a-Number, i.e., \pythonilIdx{nan}.}%
...
\pythonil{nan + 1} remains \pythonil{nan} and so does \pythonil{nan + inf}.

The value \pythonil{nan} is different from \emph{any} value.
...
Thus \pythonil{1 != 1} is obviously \pythonilIdx{False}, \pythonil{inf == inf} is \pythonilIdx{True} and \pythonil{inf != inf} is \pythonilIdx{False}.
