...
The 11~bits of the \pgls{exponent} store the resolution.
They represent a power of~2 which is multiplied to the \pgls{significand} to get the actual number.
...
Therefore, the stored value of the \pgls{exponent} is taken and a constant \pgls{bias} of~1023 is subtracted.
Thus, a stored value of 1050 in the exponent fields leads to an actual exponent of $1050-1023=27$, which would mean that the \pgls{significand} is multiplied with~$2^{27}$, i.e., $134\decSep217\decSep728$.%
%
\begin{sloppypar}%
...
You cannot represent numbers arbitrarily precisely with \pythonils{float}~\cite{PSF:P3D:TPT:FPAIAL}.
Except for that, \pythonils{float} are pretty cool, though.%
%
%
\endhsection%
%
...
Obviously, $\arcsin{\sin{0.925}}$ should be~$0.925$, since $\arcsin{\sin x} = x$~$\forall x\in[-\frac{\numberPi}{2},\frac{\numberPi}{2}]$.
Calculating \pythonil{asin(sin(0.925))}\pythonIdx{asin}\pythonIdx{sin} indeed yields~\pythonil{0.9250000000000002}.
It also holds that $\arccos{\cos x} = x$~$\forall x\in[0,\numberPi]$.
...
Never expect it to be exact~\cite{PTVF2007NRTAOSC:EAAS,BHK2006CNFCMEARFCS:NS}.%
}%
%
Due to the limited precision, it could be that you add two numbers~$a+b=c$ but then find that $c-a\neq b$, because some digit was lost. %
...
If two integer numbers are equally close to it, it returns the one that is even~\cite{PSF:P3D:TPSL:BIF}.
This rounding method is called Banker's Rounding.
It is defined in the IEEE~754 standard for floating point arithmetics~\cite{IEEE2019ISFFPA} and used in many applications today, including Alipay\textsuperscript{+}~\cite{APDOC:BR}.

This rounding behavior can be very surprising, as we learned in school that $x.5$ should be rounded to~$x+1$.
Well, I learned it like this in Germany (and \pgls{Java}'s~\pythonil{Math.round} also works like that\cite{O2024JPSEJDKV2AS}), but elsewhere it may be taught differently{\dots}
Under Banker's Rounding, this will only happen with \pythonilIdx{round} if $x+1$~is even.
The reason why Banker's Rounding is preferred is that it is not biased in any direction~\cite{MB2005RA1,SE:SO:WDP3RHTE}.
If $x.5$ is always rounded toward~$x+1$ and we have many numbers of the form $x.5$, then the average rounded result tends to be somewhat larger than the average of the unrounded numbers.
Banker's Rounding avoids this problem by sometimes rounding numbers ending in~5 to larger and sometimes to smaller values.
...
Here, \pythonil{int(11.5 + 0.5)} becomes~\pythonil{12} and \pythonil{int(12.5 + 0.5)} becomes~\pythonil{13}.
...
A \pythonilIdx{float} is accurate to between 15~and 16~decimals.
...
Such numbers~$\alpha$ are then represented in the form~$\alpha=\beta*10^{\gamma}$.
...
In other words, we write~$\beta\texttt{e}\gamma$.
In order to make sure that each number~$\alpha$ has unique representation, it is defined that $\beta$ must have exactly one non-zero leading digit, which may be followed by a decimal dot and then a fractional part.
...
Also, please be aware that this is just a notation, a way to represent numbers as text.
It is only used for input and output of numbers.
Internally, the numbers are still stored in exactly the IEEE~754~Standard format~\cite{IEEE2019ISFFPA,H1997IS7FPN}.
This is very similar to the fact that we can enter integer numbers in hexadecimal, decimal, octal, or binary format -- but regardless of how we enter them, they are still stored in the \inQuotes{same} \pythonil{int} structure, which does not remember in which format the numbers were originally specified.
...
If you need to specify large \pythonils{int} or \pythonils{float}, using underscores~(\pythonil{_}\pythonIdx{\_}) to separate groups of digits can be very helpful~\cite{PEP515}. %
...
First the easy part:
We said that \python\ can store very small numbers, like $10^{-300}$, in the \pythonilIdx{float} datatype.
But how small really?
Finding this out from the documentation is actually not so easy.
Luckily, \pgls{Java} uses the same IEEE~754~Standard\cite{IEEE2019ISFFPA} for its datatyoe \pythonil{double}.
...
Later in \cref{sec:exceptions}, we will learn what \pythonilsIdx{Exception} actually are and how to handle them properly and in \cref{sec:tryFinally}, we will circle back to exactly this \pythonilIdx{OverflowError}.
...
\pythonilIdx{inf} exists as constant in the \pythonilIdx{math} module.
...
Mathematically speaking, this is very dodgy and $\infty-\infty$ as such is not a~\inQuotes{thing}.
