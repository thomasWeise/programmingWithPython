...
%
\subfloat[][%
We enter our query \emph{\inQuotes{python rounding towards infinity}} and press~\keys{\return}.%
...
%
\subfloat[][%
Part of a screenshot obtained from the news article~\cite{S2025GAVCTLUGDT}.%
\label{fig:aiHddDeleteGolem}%
]{\tightbox{\includegraphics[width=0.47\linewidth]{\currentDir/aiHddDeleteGolem}}}%
%
\floatSep%
%
\subfloat[][%
A Part of a screenshot obtained from the news article~\cite{V2025CCGAVCUEDOOE}.%
\label{fig:aiHddDeleteRegister}%
]{\tightbox{\includegraphics[width=0.47\linewidth]{\currentDir/aiHddDeleteRegister}}}%
%
\caption{Two screenshots of news articles describing an incident where a vibe coding \pgls{AI} deleted a whole partition of data instead of a directory.~(these screenshots are not under the Creative Commons license)}%
\label{fig:aiHddDelete}%
\end{noglslink}%
\end{figure}%
%
...
\Cref{fig:aiHddDelete} provides another example where a vibe coding tool by Google wided an entire hard drive instead of a single directory~\cite{V2025CCGAVCUEDOOE,S2025GAVCTLUGDT}.
Thes examples clearly show that we need to be careful with the tools that we use.
