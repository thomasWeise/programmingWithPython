...
\pythonilIdx{None} is not equivalent to \pythonil{0}, it is not equivalent to~\pythonilIdx{nan}, and also different from the empty string~\pythonilIdx{""}.
It is just nothing.
...
This value \pythonilIdx{None} is then passed into the outer \pythonilIdx{print} function, which thus essentially does~\pythonil{print(None)} and, thus, prints \inlinelistingbox{\texttt{None}} to the console.
We therefore see two lines of text appear, first~\inlinelistingbox{\texttt{Hello World}} and then~\inlinelistingbox{\texttt{None}}.
