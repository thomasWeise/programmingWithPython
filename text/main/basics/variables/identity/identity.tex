...
Therefore, it holds that \pythonil{A == B}\pythonIdx{==} but \emph{not} \pythonil{A is B}~(i.e., it holds that \pythonil{A is not B}\pythonIdx{is not})
...
Now both \pythonil{A == C}\pythonIdx{==} and \pythonil{A is B}\pythonIdx{is} are \pythonil{True}.%
%
%
\begin{sloppypar}%
...
A simple example of the difference between equality and identity is given in program \programUrl{variables:identity_1} as \cref{lst:variables:identity_1}.
...
%
A slightly more involved example of the issue of object equality and identity is given in program \programUrl{variables:identity_2} as \cref{lst:variables:identity_2}.
You see, when the \python\ interpreter parses the program code file, it will allocate the memory and store the values of constants and \pglspl{literal}.
...
Equality is tested with the operator~\pythonilIdx{==} and inequality is tested with operator~\pythonilIdx{"!=}.
Identity is tested with operator~\pythonilIdx{is} and un-identity with operator~\pythonilIdx{is not}.
Two distinct objects can either be equal or unequal, but never identical.
Two identical objects are always equal, i.e., \pythonil{A is B}$\Rightarrow$\pythonil{A == B}.%
\endhsection%
\FloatBarrier%
\endhsection%
%
