...
Each object has a type and we already learned about several of these datatypes~(\pythonil{int}, \pythonil{bool}, \pythonil{str}, \dots) in \cref{sec:simplyDataTypesAndOperations}.
We can obtain the type of an object stored in variable \pythonil{var} by invoking \pythonil{type(var)}\pythonIdx{type}.
\Cref{lst:variables:types} shows a program \programUrl{variables:types} that does just that and the output of that program is given in \cref{exec:variables:types}.
...
%
\begin{definition}[Strongly-Typed Language]%
In a strongly-typed programming language, every \emph{value} has a \emph{type} that cannot change.%
\end{definition}%
%
Such a language does not allow interactions between values of incompatible types, such as~\pythonil{\"str\" + 5}.
Examples are \python, \pgls{Java}, and~\pgls{C}.%
%
\begin{definition}[Weakly-Typed Language]%
...
Here, the \pythonil{5} in \pythonil{\"str\" + 5} may automatically and implicitly be converted to a text string, possibly resulting in~\pythonil{\"str5\"}.
A common example is \pgls{javascript}.
%
\begin{definition}[Statically-Typed Language]%
In a statically-typed language, each \emph{variable} has a \emph{fixed type} and can only refer to values of that type.%
\end{definition}%
%
\begin{definition}[Dynamically-Typed Language]%
...
\end{definition}%
%
\python\ would be an example of a dynamically-typed language, whereas \pgls{C} and \pgls{Java} fall on the statically-typed side.
...
Let's take a look at program \programUrl{variables:types_wrong} given as \cref{lst:variables:types_wrong}.
...
Back in \cref{sec:int}, you learned that the \pythonilIdx{//} operator performs an integer division with an \pythonil{int} result, whereas the division using the \pythonilIdx{/} operator always returns a \pythonil{float}~\cite{PEP238}.
...
The type of the initial value of the variable is~\pythonil{int}.
The type of the next value of the variable is~\pythonil{float}.
The \emph{variable itself} does not have a fixed type, because the language is dynamically-typed.
Therefore storing a \pythonil{float} in a variable that presently contains an \pythonil{int} is OK.
The program executes and the output appears without error.

However, from the perspective of programming, \cref{lst:variables:types_wrong} is just plain \emph{wrong}.
...
If you read this program, then you are likely to overlook that a variable named \pythonil{int_var} contains a \pythonil{float}.
Even if you notice it, it must strike you as odd.

If you do actually notice it, you may think:~\inQuotes{Hm. Why is there a \pythonil{float} in the variable~\pythonil{int_var}?}
...
It could be that they started with code where they originally did an integer division and had only \pythonils{int} stored in the variable.
...
For example, maybe she tries to do something like~\pythonil{\"abcdefg\"[int_var]}, i.e., use \pythonil{int_var} as index in a string.
This would work fine if \pythonil{int_var} was an \pythonil{int}, but crashes the program now because \pythonil{int_var} is a \pythonil{float}.%
%
\bestPractice{codeClarity}{The names we use in program code should clearly reflect our intentions.}%
%
If the programmer had followed \cref{bp:codeClarity}, then the only way a \pythonil{float} could be in a variable named~\pythonil{int_var} would be a bug in the code.
...
The results of static type checking with \mypy\ of the program \programUrl{variables:types_wrong} given in \cref{lst:variables:types_wrong}. %
...
The results of static type checking with \mypy\ of the program \programUrl{variables:types} given in \cref{lst:variables:types}. %
(This is actually output generated by the script~\cref{lst:bash:mypy} on \cpageref{lst:bash:mypy}.)}%
%
...
Then type in \bashil{pip install mypy}\pythonIdx{Mypy}\pythonIdx{pip} and hit~\keys{\return}.
Usually, you will do that in a \pgls{virtualEnvironment}, as discussed later in \cref{sec:pipAndVenv}.
The \mypy\ tool will be installed as illustrated in \cref{fig:pipInstallMypy}.%
%
\begin{sloppypar}%
We can now apply the tool to the program \programUrl{variables:types_wrong} from \cref{lst:variables:types_wrong}.
...
In \cref{exec:variables:variable_types_wrong:mypy}, we invoke \bashil{mypy variable_types_wrong.py --no-strict-optional --check-untyped-defs}, where \programUrl{variables:types_wrong} is the (very fitting) name of the program to check.
(In some setups, this does not work, and we have to write \bashil{python3 -m mypy variable_types_wrong.py} instead of  \bashil{mypy variable_types_wrong.py}.)
...
You can then apply \mypy\ using the command \bashil{mypy fileToScan.py}, where \textil{fileToScan.py} is the name of the file to check~(you can also specify a whole directory). %
...
%
If we instead apply \mypy\ to the completely fine (albeit useless) program \programUrl{variables:types} in \cref{lst:variables:types}, it will tell us that there is no error, as illustrated in \cref{exec:variables:variable_types:mypy}.
...
Every program should pass static type checking with tools such as \mypy~(see \cref{ut:mypy}). %
...
The results of static type checking the program \programUrl{variables:assignment_wrong} with \mypy\ given in \cref{lst:variables:assignment_wrong}.}%
%
For the sake of completeness, we also apply \mypy\ to the program \programUrl{variables:assignment_wrong} given in \cref{lst:variables:assignment_wrong} that we used to illustrate the use of the \pycharm\ \pgls{ide} in finding bugs.
...
\emph{\inQuotes{\mypy\ is an additional software. %
...
There are two answers, the first one is \emph{\inQuotes{This will improve the quality of your code.}}
...
%
When we discussed the program \programUrl{variables:types_wrong} in \cref{lst:variables:types_wrong}, we stated that there could be two reasons for the error in the code:
Either, the author accidentally mixed-up the two operators~\pythonilIdx{/}~and~\pythonilIdx{//} or they chose a misleading name for their variable~\pythonil{int_var}.
The problem that any type checking tool faces is that it cannot know the intention of the programmer.
It can discover that line~4 is probably wrong, because the variable \pythonil{int_var}, which former contained an~\pythonil{int}, now gets a \pythonil{float} value assigned to it.
But it cannot find out why that happened.

Oddly enough, the problem of guessing the intention of the programmer exists only to much smaller degree in a statically typed language like~\pgls{C}.
...
It could tell us that either line~4 is wrong or our initial assignment of an \pythonil{int} to the variable in line~1 would be converted to a~\pythonil{float}\footnote{%
...
When we declare and assign a variable, \pythonil{my_var = 1} and want to define it as integer variable, for example, we simply write \pythonil{my_var: int = 1}.
...
Then, \pythonil{my_var} is a variable in which only integers should be stored.
...
The results of static type checking the program \programUrl{variables:types_wrong_hints_1} with \mypy\ of the program given in \cref{lst:variables:types_wrong_hints_1}.}%
%
...
The results of static type checking the program \programUrl{variables:types_wrong_hints_2} with \mypy\ of the program given in \cref{lst:variables:types_wrong_hints_2}.}%%
...
The \python\ intereter ignores \pglspl{typeHint}.
...
In program \programUrl{variables:types_wrong_hints_1} given as \cref{lst:variables:types_wrong_hints_1}, we annotate \pythonil{int_var} as an integer variable by writing \pythonil{int_var: int}.
...
Technically speaking, the \pgls{typeHint} specification in PEP~484~\cite{PEP484} allows all variables annotated with \pythonil{float} to also accept \pythonil{int} values in section \href{https://peps.python.org/pep-0484/\#the-numeric-tower}{The Numeric Tower}. %
Thus annotating the variable only with \pythonil{: float} would have been sufficient. %
However, since \pythonil{int} is actually \emph{not} a subclass of \pythonil{float} in \python, this can be confusing. %
Writing \pythonil{int | float} gives me the opportunity to introduce the \pythonil{|}~operator {\dots} so this is how we do it.%
}
This annotation is used in program \programUrl{variables:types_wrong_hints_2} in \cref{lst:variables:types_wrong_hints_2}.
...
Or they would have more discovered that the variable name was misleading in the process of communicating their intention by marking the variable to either hold an \pythonil{int} or a \pythonil{float}.
...
The results of static type checking with \mypy\ of the program \programUrl{variables:types_hints} given in \cref{lst:variables:types_hints}.}

We now annotate our good old program \programUrl{variables:types} from \cref{lst:variables:types} with \pglspl{typeHint} as a small exercise.
...
The variable \pythonil{str_var}, in which we want to store the string~\pythonil{\"float_var = 3.0\"}, wird mit will be annotated with \pythonil{: str}.
...
It is very obvious at this point that including the type of the variable in the variable name is not needed.
...
First, there are several important tools, like the \psycopg~\cite{VDGE2022PPDAFP:ST}, the \postgresql\ \python\ adapter, that are fully annotated with \pglspl{typeHint} based on the PEP~484~\cite{PEP484} specification.
...
Interestingly, in the case of these examples, as reason it is given that using \pglspl{typeHint} would be very complicated with their existing codebases~\cite{CFNYLH2020ST,DPVPCHG2018ATHFS}.
...
Such attacks can be prevented if the queries to \pglspl{db} are never dynamically constructed by the likes of \pglspl{fstring} but instead are always defined as string \pglspl{literal}.
\python\ supports the type~\pythonilIdx{LiteralString} for string \pglspl{literal}~\cite{PEP675}.
...
And in this section, you have learned two more building blocks to do just that, in the dynamically-typed environment of \python.
Type checking and type hints.
We add them to proper commenting of code, following style conventions, carefully checking your \pgls{ide} for warnings, and properly reading any output of crashed programs to locate the erros.
