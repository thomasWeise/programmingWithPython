...
%
\begin{sloppypar}%
We can now apply the tool to the program \textil{variable_types_wrong.py} from \cref{lst:variables:types_wrong}.
...
%
If we instead apply \mypy\ to the completely fine (albeit useless) program \textil{variable_types.py} in \cref{lst:variables:types}, it will tell us that there is no error, as illustrated in \cref{exec:variables:variable_types:mypy}.
...
%
For the sake of completeness, we also apply \mypy\ to the program \textil{assignment_wrong.py} given in \cref{lst:variables:assignment_wrong} that we used to illustrate the use of the \pycharm\ \pgls{ide} in finding bugs.
...
%
When we discussed the program \textil{variable_types_wrong.py} in \cref{lst:variables:types_wrong}, we stated that there could be two reasons for the error in the code:
...
We now annotate our good old program \textil{variable_types.py} from \cref{lst:variables:types} with \pglspl{typeHint} as a small exercise.
