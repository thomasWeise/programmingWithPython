...
A \python\ program showing several steps of the approximation of $\pi$ using the method of LIU Hui.}%
%
\begin{figure}[tb]%
\centering%
%
...
We print the approximated value of~$\pi$ as \pythonil{e * s / 2}.
Notice how elegantly we use the unicode character~$\pi$ via the escape~\pythonil{\\u03c0}\pythonIdx{\textbackslash{u}} from back in~\cref{sec:unicodeChars} (and how nicely it indeed prints the greek character~$\pi$ in the standard output in \cref{exec:variables:pi_liu_hui}).
Either way, since \cref{eq:liuhui:sidelength,eq:liuhui:approx} are always the same, we can simply copy-paste the lines of code for updating~\pythonil{s}, \pythonil{e}, and printing the approximated value of~$\pi$ several times.
